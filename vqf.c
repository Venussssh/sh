...
void VQFParams_Init(vqf_real_t gyrTs, vqf_real_t accTs, vqf_real_t magTs)
{
    ...
    params.motionBiasEstEnabled = false;//陀螺仪偏差估计，源码true为启用，fause为弃用
#endif

    params.restBiasEstEnabled = true;//Enables rest detection and gyroscope bias estimation during rest phases.
    params.magDistRejectionEnabled = false;//关闭抗磁干扰
    params.biasSigmaInit = 0.5;
    params.biasForgettingTime = 100.0;
    params.biasClip = 2.0;
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    params.biasSigmaMotion = 0.1;
    params.biasVerticalForgettingFactor = 0.0001;
#endif
    params.biasSigmaRest = 0.03;
    params.restMinT = 1.5;//休息检测的时间阈值
    params.restFilterTau = 0.5;//二阶巴特沃斯低通滤波器
    params.restThGyr = 2.0;//静止检测的时间阈值
    params.restThAcc = 0.5;
    params.magCurrentTau = 0.05;
    params.magRefTau = 20.0;
    params.magNormTh = 0.1;
    params.magDipTh = 10.0;
    params.magNewTime = 20.0;
    params.magNewFirstTime = 5.0;
    params.magNewMinGyr = 20.0;
    params.magMinUndisturbedTime = 0.5;
    params.magMaxRejectionTime = 60.0;
    params.magRejectionFactor = 2.0;
...
void updateGyr(const vqf_real_t gyr[3])
{
    ...
    if (params.restBiasEstEnabled || params.magDistRejectionEnabled) {
        ...
        vqf_real_t biasClip = params.biasClip*(M_PI/180.0);
        if (state.restLastSquaredDeviations[0] >= square(params.restThGyr*(M_PI/180.0))
...
void updateAcc(const vqf_real_t acc[3])
{
    ...
    if (params.motionBiasEstEnabled || params.restBiasEstEnabled) {
        ...
        if (state.restDetected && params.restBiasEstEnabled) {
            ...
            // std_fill(w, w+3, coeffs.biasRestW);
            std_fill_any(w, 3, coeffs.biasRestW);
            ...
            // std_fill(w, w+3, -1); // disable update
            std_fill_any(w, 3, -1);
        }
...
void setup()
{
    ...
    //std_max使用函数
    coeffs.biasVerticalW = coeffs.biasMotionW / std_max(params.biasVerticalForgettingFactor, 1e-10);
#endif
    ...
    {
        std_fill(coeffs.magNormDipLpB, 3);
        std_fill(coeffs.magNormDipLpA, 2);
    }
    resetState();
...
void resetState()
{
    ...
    std_fill_zero(state.lastAccLp, 3);
    // std::fill(state.accLpState, state.accLpState + 3*2, NaN);
    std_fill(state.accLpState, 3*2);
    ...
    std_fill_zero(state.magNormDip, 2);
    // std::fill(state.magNormDipLpState, state.magNormDipLpState + 2*2, NaN);
    std_fill_doublezero(state.magNormDipLpState, 2*2);
}
...
void filterCoeffs(vqf_real_t tau, vqf_real_t Ts, double outB[], double outA[])
{
    ...
    double C = tan(M_PI*fc*(Ts));//少了double
...
vqf_real_t getBiasEstimate(vqf_real_t out[3])
{
    ...
    return sqrt(P)*(M_PI/100.0/180.0);
}
...
void setBiasEstimate(vqf_real_t bias[3], vqf_real_t sigma)
{
    ...
    std_copy(bias, 3, state.bias);
    if (sigma > 0) {
        vqf_real_t P = square(sigma*(180.0*100.0/M_PI));
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
...
{
    out[0] = sqrt(state.restLastSquaredDeviations[0]) / (params.restThGyr*(M_PI/180.0));
...
void setMotionBiasEstEnabled(bool enabled)
{
    ...
    // std::fill(state.motionBiasEstRLpState, state.motionBiasEstRLpState + 9*2, NaN);
    std_fill(state.motionBiasEstRLpState, 9*2);
    // std::fill(state.motionBiasEstBiasLpState, state.motionBiasEstBiasLpState + 2*2, NaN);
    std_fill(state.motionBiasEstBiasLpState, 2*2);
}
#endif
...
void setMagDistRejectionEnabled(bool enabled)
{
    ...
    state.magCandidateT = 0.0;
    // std::fill(state.magNormDipLpState, state.magNormDipLpState + 2*2, NaN);
    std_fill(state.magNormDipLpState, 2*2);
}
...
void setTauAcc(vqf_real_t tauAcc)
{
    ...
    std_copy(newA, 3, coeffs.accLpB);
    // std::copy(newA, newA+2, coeffs.accLpA);
    std_copy(newA, 2, coeffs.accLpA);
}
