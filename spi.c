...
7.31前代码：
 ...
 uint8_t SPI2_Read(uint8_t address)
 {
    ...
    _CS_ON();
    HAL_SPI_TransmitReceive(&hspi2,txBuf,rxBuf,sizeof(txBuf),500);
    _CS_OFF();
...
{
  address &= 0x7F;
  uint8_t txBuf[2] = { address, text };

  _CS_ON();
  HAL_SPI_Transmit(&hspi2,txBuf,sizeof(txBuf),500);
  _CS_OFF();
...
//多位读函�?
void SPI_Read_Mul_Bite(uint8_t address_start)
{
  address_start |= 0x80;
  //uint8_t temp = address_start;
  //unsigned char *padd;
  //padd = (unsigned char *)address_start;

  uint8_t txBuf[7] = { address_start};
  uint8_t rxBuf[7] = {0};
  _CS_ON();
  HAL_SPI_TransmitReceive(&hspi2, txBuf, rxBuf, 7, 500);
  _CS_OFF();
...
{
  address_start |= 0x80;                        // read
  uint8_t txBuf[7] = { address_start , 0 };
  _CS_ON();
  HAL_SPI_TransmitReceive(&hspi2, txBuf, rev_data, 7, 500);
  _CS_OFF();
